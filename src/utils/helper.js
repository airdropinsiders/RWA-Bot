...
Balance                 : ${data.eth} ${RPC.SYMBOL}
                          ${data.trwa} TRWA
                          ${data.usdc} USDC
Today (TRWA|USDC|STAKE) : ${data.todayTrwa}x | ${data.todayUsdc}x | ${data.todayStake}x
  ...
  static delay = (ms, acc, msg, obj) => {
    return new Promise(async (resolve) => {
      ...
      if (acc != undefined) {
        ...
        await this.twist.info(`Delaying for ${this.msToTime(ms)}`);
      }
